...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, inputs, filter_shape, image_shape, W=None, b=None, poolsize=(2, 2)):
        ...
        if W is None:
            ...
            self.W = tf.Variable(np.asarray(rng.uniform(low=-W_bound, high=W_bound, 
                                                                          size=filter_shape),dtype='float32'))
        else:
        ...
        self.W = tf.transpose(W,perm=[3,2,1,0])
        conv_out = tf.nn.conv2d(self.inputs, self.W, strides=(1,1,1,1), padding="VALID",data_format=data_format)

        # downsample each feature map individually, using maxpooling   *******
        pooled_out = tf.nn.max_pool(conv_out, ksize=[1,1,2,2], strides=[1,1,2,2], padding='VALID',data_format=data_format)

        self.output = tf.maximum(0.0, pooled_out + self.b)
