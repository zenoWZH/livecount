...
if __name__ == '__main__':
    ...
    #for vidNum in range(1,num_of_vids+1):
        ...
        #video_writer = cv2.VideoWriter('../DeepRepICCV2015/out/YTIO_out'+str(vidNum)+'.avi', fourcc, frame_rate, (640, 480))
    for vidNum in data_list:
        try:
            cap = cv2.VideoCapture(data_route+str(vidNum))    
            # output video
            video_writer = cv2.VideoWriter(out_route+str(vidNum), cv2.VideoWriter_fourcc(*'XVID'), frame_rate, (640, 480))    
        ...
        while True:
            ...
            if ((cur_state == state.COOLDOWN) and (global_counter>=5)):
                ...
                countMap[videonum-1,actions_counter-1] = global_counter
